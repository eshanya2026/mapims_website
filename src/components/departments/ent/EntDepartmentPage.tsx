...
const serviceIcons = [Ear, Wind, Mic, Baby, Flower2] as const;
const surgeryIcons = [Ear, ScanLine, Wind, Baby, Mic] as const;
      ...
      <section className="relative flex min-h-[50vh] items-center overflow-hidden md:min-h-[55vh]">
        ...
        <div className="container relative z-20 mx-auto section-padding px-4">
          ...
          >
            ...
            <DepartmentHeroBadge>Best ENT hospital in India</DepartmentHeroBadge>
            <h1 className="text-4xl font-bold leading-tight text-white sm:text-5xl md:text-6xl">
              <span className="text-red-500">Otorhinolaryngology — ENT</span>
            </h1>
            <p className="mt-4 max-w-2xl text-lg leading-relaxed text-slate-300">
              Discover exceptional ear, nose, and throat care — from routine
              consultation to advanced endoscopic and head & neck surgery.
            </p>
          ...
          <div className="order-1 min-w-0 flex-1 lg:order-2">
            ...
            >
              <div className="text-center">
                ...
                <h2 className="mb-4 text-3xl font-bold text-slate-900 md:text-4xl">
                  Comprehensive <span className="text-red-600">ENT Services</span>
                </h2>
                <p className="mx-auto mb-10 max-w-2xl leading-relaxed text-slate-600">
                  {entServicesIntro}
                </p>
                <div className="mx-auto grid max-w-6xl grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                  {entServices.map((item, index) => {
                    const Icon = serviceIcons[index] ?? Ear;
                    return (
                      <motion.article
                        key={item.title}
                        initial={{ opacity: 0, y: 20 }}
            ...
            >
              <div className="mb-10 text-center">
                ...
                <h2 className="mb-4 text-3xl font-bold text-slate-900 md:text-4xl">
                  ENT <span className="text-red-600">Surgeries</span>
                </h2>
              ...
              <div className="mx-auto grid max-w-6xl grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {entSurgeries.map((procedure, index) => {
            ...
            >
              <div className="mb-10 text-center">
                ...
                <h2 className="mb-4 text-3xl font-bold text-slate-900 md:text-4xl">
                  Cutting-Edge <span className="text-red-600">ENT Technology</span>
                </h2>
              </div>
              <div className="mx-auto grid max-w-6xl grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {entTechnology.map((tech, index) => {
            ...
            >
              <div className="max-w-3xl">
                ...
                <h2 className="mb-4 text-3xl font-bold text-slate-900 md:text-4xl">
                  {entSpeciality.heading}
                </h2>
                <div className="space-y-4 leading-relaxed text-slate-600">
                  <p>{entSpeciality.preview}</p>
                  <p>{entSpeciality.full}</p>
                </div>
              </div>
              <ul className="mt-8 flex flex-wrap gap-2">
                {entSpecialityAreas.map((area) => (
                  <li
                    key={area}
            ...
            >
              ...
              <div className="relative z-10 mx-auto -mt-6 grid max-w-6xl grid-cols-1 gap-5 px-2 sm:grid-cols-2 md:-mt-8 md:gap-6 md:px-4 lg:grid-cols-3">
                {entInfrastructure.map((item, index) => {
              ...
              >
                ...
                <div>
                  <h3 className="text-lg font-bold text-red-900">{entTeamNote.title}</h3>
                  <p className="mt-2 text-sm leading-relaxed text-red-800/80 md:text-base">
                    {entTeamNote.description}
                  </p>
                </div>
              </motion.div>
            </section>
