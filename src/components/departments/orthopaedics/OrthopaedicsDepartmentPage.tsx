...
import {
  ...
  orthopaedicsServicesIntro,
  orthopaedicsSpeciality,
  orthopaedicsSpecialityAreas,
  orthopaedicsInfrastructure,
  orthopaedicsAchievementsNote,
  orthopaedicsTeamNote,
  orthopaedicsJourney,
  orthopaedicsJourneyPillars,
  orthopaedicsHeroImage,
  orthopaedicsSpecialists,
  ...
  return (
    <main className="min-h-screen">
      <section className="relative flex min-h-[50vh] items-center overflow-hidden md:min-h-[55vh]">
        <HeroBackground imageSrc={orthopaedicsHeroImage} />
        <div className="container relative z-20 mx-auto section-padding px-4">
          ...
          >
            <nav className="mb-6 flex flex-wrap items-center gap-2 text-sm text-white/70">
              ...
              <ChevronRight className="h-4 w-4" />
              <span className="font-medium text-white">Orthopaedics</span>
            </nav>
            <DepartmentHeroBadge>Best orthopaedics in India</DepartmentHeroBadge>
            <h1 className="text-4xl font-bold leading-tight text-white sm:text-5xl md:text-6xl">
              <span className="text-red-500">Orthopaedics Department</span>
            </h1>
            <p className="mt-4 max-w-2xl text-lg leading-relaxed text-slate-300">
              World-class musculoskeletal care — from fractures and sports injuries
              to joint replacement and spine surgery.
            </p>
          ...
          <div className="order-1 min-w-0 flex-1 lg:order-2">
            ...
            >
              ...
              <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5">
                {whyChooseOrthopaedics.map((item, index) => (
                  <motion.article
                    key={item.title}
            ...
            >
              <div className="text-center">
                ...
                <div className="mx-auto grid max-w-6xl grid-cols-1 gap-6 sm:grid-cols-2">
                  {orthopaedicsServices.map((service, index) => {
                    const Icon = serviceIcons[index] ?? Bone;
                    return (
                      <motion.article
                        key={service.title}
                        initial={{ opacity: 0, y: 20 }}
            ...
            >
              <div className="max-w-3xl">
                ...
                <h2 className="mb-4 text-3xl font-bold text-slate-900 md:text-4xl">
                  Comprehensive <span className="text-red-600">Orthopaedic Services</span>
                </h2>
                <div className="space-y-4 leading-relaxed text-slate-600">
                  <p>{orthopaedicsSpeciality.preview}</p>
                  <p>{orthopaedicsSpeciality.full}</p>
                </div>
              </div>
              <div className="mt-8 flex flex-wrap gap-2">
                {orthopaedicsSpecialityAreas.map((area) => (
                  <span
                    key={area}
            ...
            >
              <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-red-800 via-red-700 to-red-900 px-8 py-10 md:px-12 md:py-14">
                <div className="relative max-w-3xl">
                  ...
                  <h2 className="mt-2 text-3xl font-bold text-white md:text-4xl">
                    State-of-the-Art <span className="text-red-200">Facilities</span>
                  </h2>
                  <p className="mt-4 leading-relaxed text-red-50/90">
                    Modular operation theatres, critical care units, in-house diagnostics,
                    and comfortable recovery spaces for every orthopaedic patient.
                  </p>
                </div>
              </div>
              ...
              <div className="relative z-10 mx-auto -mt-6 grid max-w-6xl grid-cols-1 gap-5 px-2 sm:grid-cols-2 md:-mt-8 md:gap-6 md:px-4 lg:grid-cols-3">
                {orthopaedicsInfrastructure.map((item, index) => {
            ...
            <div id="specialists" className="scroll-mt-28 border-b border-slate-100">
              <DepartmentSpecialistDoctors
                ...
                specialists={orthopaedicsSpecialists}
                embedded
              />
            </div>
