...
import {
  ...
  obgynServicesIntro,
  obgynSpeciality,
  gynaecologySpecialityAreas,
  obstetricsSpecialityAreas,
  obgynInfrastructure,
  obgynAchievementsNote,
  obgynTeamNote,
  obgynJourney,
  obgynJourneyPillars,
  obgynHeroImage,
...
const serviceIcons = [Baby, Heart, Stethoscope, Activity, Shield] as const;
...
function SpecialityTagList({ title, items }: { title: string; items: string[] }) {
  return (
    <div>
  ...
  return (
    <main className="min-h-screen">
      <section className="relative flex min-h-[50vh] items-center overflow-hidden md:min-h-[55vh]">
        <HeroBackground imageSrc={obgynHeroImage} />
        <div className="container relative z-20 mx-auto section-padding px-4">
          ...
          >
            <nav className="mb-6 flex flex-wrap items-center gap-2 text-sm text-white/70">
              ...
              <ChevronRight className="h-4 w-4" />
              <span className="font-medium text-white">Obstetrics & Gynaecology</span>
            </nav>
            <DepartmentHeroBadge>Best obstetrics & gynaecology in India</DepartmentHeroBadge>
            <h1 className="text-4xl font-bold leading-tight text-white sm:text-5xl md:text-6xl">
              <span className="text-red-500">Obstetrics & Gynaecology</span>
            </h1>
            <p className="mt-4 max-w-2xl text-lg leading-relaxed text-slate-300">
              Exceptional women's healthcare — from pregnancy and fertility to
              surgery, neonatal care, and lifelong gynaecological support.
            </p>
          ...
          <div className="order-1 min-w-0 flex-1 lg:order-2">
            ...
            >
              <div className="max-w-3xl">
                ...
                <h2 className="mb-4 text-3xl font-bold text-slate-900 md:text-4xl">
                  Comprehensive <span className="text-red-600">Care & Services</span>
                </h2>
            ...
            >
              <div className="max-w-3xl">
                ...
                <h2 className="mb-4 text-3xl font-bold text-slate-900 md:text-4xl">
                  {obgynSpeciality.heading}
                </h2>
                <div className="space-y-4 leading-relaxed text-slate-600">
                  <p>{obgynSpeciality.preview}</p>
                  <p>{obgynSpeciality.full}</p>
                </div>
              </div>
              <div className="mt-10 space-y-10">
                <SpecialityTagList
                  title="Gynaecology Services"
                  items={gynaecologySpecialityAreas}
                />
                <SpecialityTagList
                  title="Obstetrics Services"
                  items={obstetricsSpecialityAreas}
                />
              </div>
            </section>
            ...
            >
              <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-red-800 via-red-700 to-red-900 px-8 py-10 md:px-12 md:py-14">
                <div className="relative max-w-3xl">
                  ...
                  <h2 className="mt-2 text-3xl font-bold text-white md:text-4xl">
                    Advanced <span className="text-red-200">Maternal & Neonatal Care</span>
                  </h2>
                  <p className="mt-4 leading-relaxed text-red-50/90">
                    Labor rooms, NICU, critical care, and in-house diagnostics for safe
                    deliveries and women's surgical care.
                  </p>
                </div>
              </div>
              ...
              <div className="relative z-10 mx-auto -mt-6 grid max-w-6xl grid-cols-1 gap-5 px-2 sm:grid-cols-2 md:-mt-8 md:gap-6 md:px-4 lg:grid-cols-3">
                {obgynInfrastructure.map((item, index) => {
              ...
              >
                ...
                <div>
                  <h3 className="text-lg font-bold text-red-900">{obgynTeamNote.title}</h3>
                  <p className="mt-2 text-sm leading-relaxed text-red-800/80 md:text-base">
                    {obgynTeamNote.description}
                  </p>
                </div>
              </motion.div>
            </section>
